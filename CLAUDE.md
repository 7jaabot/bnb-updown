...
- **[0]** → lance les 17 stratégies en parallèle (ParallelBot, paper only)
- **[1-17]** → stratégie individuelle
- **"5,11,7"** → combined strategy (consensus)
- **"3,5,11|5,3,15|1,2,10,5"** → multi-combined (N combos en parallèle, un seul process, live OK)
...
- `scripts/recover_claims.py` — one-shot recovery des claims non-réclamés (dry-run par défaut)
...
## Safety (live mode)
- **Instance guard** : PID lock file — un seul process live par wallet (`logs/live/.coordination/wallet-*.pid`)
- **Drawdown circuit breaker** : stop si balance < $50 OU drawdown ≥ 50% du peak, cooldown 60 min
- **Coordination state** : `verify_transaction()` met à jour "confirmed"/"failed", stale detection >5 min
- **Claim sweep** : `sweep_unclaimed_wins()` cross-instance au startup + toutes les 5 min, purge >24h
- **Multi-combined consensus** : unanimité requise — si conflit UP/DOWN entre combos → skip
