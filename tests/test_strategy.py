...
class TestComputePositionSize:
    def test_bankroll_pct_sizing(self):
        """5% of bankroll should be the base position size."""
        size = compute_position_size(bankroll=1000.0, bankroll_pct=0.05)
        assert size == pytest.approx(50.0)

    def test_bankroll_pct_sizing_small(self):
        """5% of small bankroll."""
        size = compute_position_size(bankroll=200.0, bankroll_pct=0.05)
    ...
    def test_max_pool_pct_cap(self):
        ...
        size = compute_position_size(
            bankroll=10000.0,            # 5% = 500
            bankroll_pct=0.05,
            max_pool_pct=0.10,
            pool_total_usdc=50.0,        # 10% of pool = 5
        )
        assert size == pytest.approx(5.0)

    def test_min_usdc_skip_when_below(self):
        """If position < min_usdc, trade is skipped (returns 0)."""
        size = compute_position_size(
            bankroll=80.0,               # 5% = 4 < min 5
            bankroll_pct=0.05,
            min_usdc=5.0,
        )
    ...
    def test_pool_total_zero_skips_pool_cap(self):
        """When pool_total_usdc=0, pool% cap should not apply."""
        size = compute_position_size(
            bankroll=1000.0,
            bankroll_pct=0.05,
    ...
    def test_rounding(self):
        ...
        size = compute_position_size(bankroll=333.33, bankroll_pct=0.05)
        assert size == pytest.approx(16.67)
